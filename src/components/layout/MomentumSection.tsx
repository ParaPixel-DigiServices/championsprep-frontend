...
export default function MomentumSection() {
    ...
    const ctx = gsap.context(() => {
      ...
      steps.forEach((step) => {
        gsap.fromTo(`.step-circle-${step.id}`,
          { backgroundColor: "#FFFDF5", color: "#ddd", borderColor: "#ddd" },
          { 
            backgroundColor: "#3b1578", color: "#ffffff", borderColor: "#3b1578",
            duration: 0.5,
            scrollTrigger: {
  ...
  return (
    <section ref={containerRef} className="py-24 md:py-32 px-5 md:px-12 bg-[#FFFDF5] overflow-hidden relative z-30">
            ...
            {steps.map((step, index) => (
              <div key={step.id} className={`step-row-${step.id} relative grid grid-cols-1 md:grid-cols-12 gap-8 md:gap-12 items-start md:items-center`}>
                ...
                <div className="col-span-12 md:col-span-1 flex items-start md:justify-center relative z-10">
                  <div className={`step-circle-${step.id} w-10 h-10 md:w-14 md:h-14 rounded-full border-2 border-gray-200 bg-[#FFFDF5] text-gray-300 flex items-center justify-center font-bold text-lg md:text-xl shadow-[0_0_0_8px_#FFFDF5] transition-colors`}>
                    {step.number}
                  </div>
                </div>
                ...
                <div className={`step-card-${step.id} col-span-12 md:col-span-5 pl-12 md:pl-0 flex md:justify-end`}>
                  <div className={`floating-card floating-card-delay-${index + 1} bg-white p-5 md:p-6 rounded-2xl shadow-[0_15px_30px_rgba(6,0,39,0.1)] border border-[#060027]/5 flex items-center gap-4 max-w-xs transform ${step.card.rotate}`}>
